...
# リモートで消えたブランチのローカル追跡情報を削除（prune）
git fetch -p
----

[source,bash]
.origin/main からの全差分を新しいコミットで打ち消す
----
git revert origin/main..HEAD --no-commit
git commit -m "revert to origin/main state"
----
...
--
[horizontal]
`-n`::: dry-run（削除対象を表示するだけで実際には削除しない）
`-f`::: 強制削除（untracked ファイルを削除）
`-d`::: ディレクトリも対象に含める
--
...
.diff
----
# 特定のブランチやコミット間のステータスを見る場合（例：mainとfeatureブランチ）
git diff --name-status main feature

# M  src/foo.ts    # Modified
# A  src/bar.ts    # Added
# D  src/baz.ts    # Deleted
# R  old → new     # Renamed
# C  src/copy.ts   # Copied
----
...
# by: https://gigazine.net/news/20260410-git-command-before-reading-code/
----
